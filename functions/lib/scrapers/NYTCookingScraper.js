...
exports.NYTCookingScraper = void 0;
const BaseScraper_1 = require("./BaseScraper");
class NYTCookingScraper extends BaseScraper_1.BaseScraper {
    canHandle(hostname) {
    ...
    detectPaywall($) {
        ...
        return paywallIndicators.some(indicator => paywallText.toLowerCase().includes(indicator));
    }
    extractFromJsonLd($) {
        try {
    ...
    extractFromSelectors($) {
        try {
            ...
            const title = this.extractText($('h1[data-testid="recipe-title"], ' +
                '.recipe-title, ' +
                '.nyt5-headline, ' +
            ...
            if (!title)
                return null;
            const description = this.extractText($('[data-testid="recipe-description"], ' +
                '.recipe-intro, ' +
                '.nyt5-summary, ' +
                '.css-1wev1x0, ' + // NYT description class
                '.recipe-summary').first());
            // NYT image extraction - they often use specific image containers
            const imageElement = $('[data-testid="recipe-image"] img, ' +
                '.recipe-photo img, ' +
                '.nyt5-image img, ' +
                '.css-1l1j2ho img, ' + // NYT image container
                '.media-viewer img').first();
            const image = imageElement.attr('src') ||
                imageElement.attr('data-src') ||
                imageElement.attr('data-lazy-src');
            // NYT ingredients - they have specific ingredient list structures
            const ingredientElements = $('[data-testid="recipe-ingredients"] li, ' +
            ...
            const instructionElements = $('[data-testid="recipe-instructions"] li, ' +
            ...
            const prepTimeText = this.extractText($('[data-testid="recipe-time-prep"], ' +
                '.recipe-time-prep, ' +
                '.prep-time, ' +
                '.nyt5-time .prep').first());
            const cookTimeText = this.extractText($('[data-testid="recipe-time-cook"], ' +
                '.recipe-time-cook, ' +
                '.cook-time, ' +
                '.nyt5-time .cook').first());
            const totalTimeText = this.extractText($('[data-testid="recipe-time-total"], ' +
                '.recipe-time-total, ' +
                '.total-time, ' +
                '.nyt5-time .total').first());
            ...
            const servingsText = this.extractText($('[data-testid="recipe-yield"], ' +
                '.recipe-yield, ' +
                '.servings, ' +
                '.nyt5-yield, ' +
                '.serves').first());
...
exports.NYTCookingScraper = NYTCookingScraper;
//# sourceMappingURL=NYTCookingScraper.js.map