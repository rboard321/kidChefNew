...
exports.FoodNetworkScraper = void 0;
const BaseScraper_1 = require("./BaseScraper");
const jsonLdNormalizer_1 = require("../services/jsonLdNormalizer");
class FoodNetworkScraper extends BaseScraper_1.BaseScraper {
    canHandle(hostname) {
    ...
    extractFromSelectors($) {
        try {
            ...
            const title = this.extractText($('.o-AssetTitle__a-HeadlineText, ' +
                'h1.entry-title, ' +
                '.recipe-title, ' +
            ...
            const description = this.extractText($('.o-AssetSummary__a-Description, ' +
                '.recipe-summary, ' +
                '.entry-summary, ' +
                '.o-RecipeInfo__a-Description, ' +
                '[data-module="RecipeSummary"]').first());
            // Enhanced image extraction with more selector options
            const imageElement = $('.m-MediaBlock__a-Image img, ' +
                '.recipe-image img, ' +
                '.entry-image img, ' +
                '.o-MediaBlock__a-Image img, ' +
                '.recipe-lead-image img, ' +
                '[data-module="RecipeImage"] img').first();
            const image = imageElement.attr('src') ||
                imageElement.attr('data-src') ||
                imageElement.attr('data-lazy-src');
            // Enhanced ingredient extraction with multiple selector strategies
            const ingredientElements = $('.o-RecipeIngredients__a-Ingredient, ' +
                '.recipe-ingredient, ' +
                '.ingredients li, ' +
            ...
            const instructionElements = $('.o-Method__m-Step, ' +
                '.recipe-instruction, ' +
                '.directions li, ' +
                '.instructions li, ' +
...
exports.FoodNetworkScraper = FoodNetworkScraper;
