...
exports.SimplyRecipesScraper = void 0;
const BaseScraper_1 = require("./BaseScraper");
class SimplyRecipesScraper extends BaseScraper_1.BaseScraper {
    canHandle(hostname) {
    ...
    async scrape($, html) {
        try {
            ...
            return {
                recipe,
                confidence: Math.min(confidence + 0.04, 1.0),
                method,
    ...
    extractFromSelectors($) {
        try {
            ...
            const title = this.extractText($('h1.entry-title, ' +
                'h1.recipe-title, ' +
                '.recipe-header h1, ' +
            ...
            const prepTimeText = this.extractText($('.recipe-prep-time, ' +
                '.prep-time, ' +
                '.recipe-time .prep-time, ' +
                '[data-cy="prep-time"], ' +
                '.timing-prep').first());
            const cookTimeText = this.extractText($('.recipe-cook-time, ' +
                '.cook-time, ' +
                '.recipe-time .cook-time, ' +
                '[data-cy="cook-time"], ' +
                '.timing-cook').first());
            const totalTimeText = this.extractText($('.recipe-total-time, ' +
                '.total-time, ' +
            ...
            const servingsText = this.extractText($('.recipe-yield, ' +
                ...
                '.recipe-serving-size').first());
            const servings = this.extractNumber(servingsText);
            // Simply Recipes often includes difficulty level
            const difficultyText = this.extractText($('.recipe-difficulty, ' +
                '.difficulty, ' +
                '.skill-level, ' +
                '.recipe-skill-level').first());
            return {
                title,
...
exports.SimplyRecipesScraper = SimplyRecipesScraper;
//# sourceMappingURL=SimplyRecipesScraper.js.map