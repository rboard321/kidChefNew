...
export class BBCGoodFoodScraper extends BaseScraper {
  ...
  async scrape($: cheerio.CheerioAPI, html: string): Promise<ScraperResult> {
    try {
      ...
      return {
        recipe,
        confidence: Math.min(confidence + 0.03, 1.0), // Bonus for BBC quality
        method,
        issues
      };
    } catch (error) {
  ...
  private extractFromSelectors($: cheerio.CheerioAPI): Partial<ScrapedRecipe> | null {
    try {
      ...
      const totalTimeText = this.extractText($(
        '.recipe-details__cooking-time-total, ' +
