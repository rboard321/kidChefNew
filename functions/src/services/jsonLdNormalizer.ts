...
export interface NormalizationResult {
  recipe: any; // Normalized JSON-LD recipe object
  improved: boolean; // Whether normalization made changes
  issues: string[]; // Any issues found during normalization
}
...
export class JsonLdNormalizer {
   ...
   */
  static normalize(jsonLdData: any, hostname: string): NormalizationResult {
    const issues: string[] = [];
    let improved = false;
    let recipe = { ...jsonLdData };
    ...
    try {
      ...
      return {
        recipe: jsonLdData, // Return original on error
        improved: false,
   ...
   */
  private static normalizeFoodNetwork(recipe: any): NormalizationResult {
    const issues: string[] = [];
    let improved = false;
    ...
    try {
      ...
      if (recipe.recipeIngredient && Array.isArray(recipe.recipeIngredient)) {
        const normalizedIngredients = recipe.recipeIngredient.map((ingredient: any) => {
   ...
   */
  private static normalizeBBCGoodFood(recipe: any): NormalizationResult {
    const issues: string[] = [];
    let improved = false;
    ...
    try {
      ...
      if (recipe.recipeIngredient && Array.isArray(recipe.recipeIngredient)) {
        const flattenedIngredients: string[] = [];
      ...
      if (recipe.recipeIngredient) {
        ...
        recipe.recipeIngredient = recipe.recipeIngredient.map((ingredient: string) => {
   ...
   */
  private static normalizeSimplyRecipes(recipe: any): NormalizationResult {
    const issues: string[] = [];
    let improved = false;
    ...
    try {
      ...
      if (recipe.recipeInstructions && Array.isArray(recipe.recipeInstructions)) {
        const cleanedInstructions = recipe.recipeInstructions.filter((instruction: any) => {
   ...
   */
  private static normalizeFood52(recipe: any): NormalizationResult {
    const issues: string[] = [];
    let improved = false;
    ...
    try {
      ...
      if (recipe.recipeInstructions && Array.isArray(recipe.recipeInstructions)) {
        const cleanedInstructions = recipe.recipeInstructions.map((instruction: any) => {
          ...
          return typeof instruction === 'string' ? text : { ...instruction, text };
        });
      ...
      if (recipe.recipeIngredient && Array.isArray(recipe.recipeIngredient)) {
        const normalizedIngredients = recipe.recipeIngredient.map((ingredient: string) => {
   ...
   */
  private static normalizeGeneric(recipe: any): NormalizationResult {
    const issues: string[] = [];
    let improved = false;
    ...
    try {
      ...
      if (recipe.recipeInstructions && Array.isArray(recipe.recipeInstructions)) {
        ...
        recipe.recipeInstructions = recipe.recipeInstructions.filter((instruction: any) => {
          const text = typeof instruction === 'string' ? instruction : instruction.text || instruction.name || '';
      ...
      if (recipe.recipeIngredient && Array.isArray(recipe.recipeIngredient)) {
        ...
        recipe.recipeIngredient = recipe.recipeIngredient.filter((ingredient: string) => {
  ...
  private static normalizeTimeString(timeStr: string): string {
    ...
    const patterns = [
      { regex: /(\d+)\s*hours?\s*(\d+)\s*minutes?/i, format: (h: string, m: string) => `PT${h}H${m}M` },
      { regex: /(\d+)\s*hrs?\s*(\d+)\s*mins?/i, format: (h: string, m: string) => `PT${h}H${m}M` },
      { regex: /(\d+)\s*hours?/i, format: (h: string) => `PT${h}H` },
      { regex: /(\d+)\s*hrs?/i, format: (h: string) => `PT${h}H` },
      { regex: /(\d+)\s*minutes?/i, format: (m: string) => `PT${m}M` },
      { regex: /(\d+)\s*mins?/i, format: (m: string) => `PT${m}M` }
    ];

    for (const pattern of patterns) {
      const match = timeStr.match(pattern.regex);
      if (match) {
        return pattern.format.apply(null, match.slice(1) as any);
      }
    }
