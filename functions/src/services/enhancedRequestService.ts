...
interface RequestOptions {
  ...
  retries?: number;
  delay?: number;
  userAgent?: 'random' | 'chrome' | 'firefox' | 'safari';
}
...
export class EnhancedRequestService {
  ...
  async fetchWithRetry(url: string, options: RequestOptions = {}): Promise<RequestResult> {
    const {
      timeout = 15000,
      retries = 3,
      delay = 2000,
  ...
  private selectUserAgent(type: string, attempt: number): string {
    if (type === 'random') {
      const browsers = ['chrome', 'firefox', 'safari'];
      const browser = browsers[attempt % browsers.length] as keyof typeof this.userAgents;
      const agents = this.userAgents[browser];
      return agents[attempt % agents.length];
    }

    if (type in this.userAgents) {
      const agents = this.userAgents[type as keyof typeof this.userAgents];
      return agents[attempt % agents.length];
    }
  ...
  private generateHeaders(userAgent: string, url: string, attempt: number): Record<string, string> {
  ...
  private isBlockedResponse(response: AxiosResponse): boolean {
  ...
  generateReferer(url: string): string {
    try {
...
// Singleton instance
export const enhancedRequestService = new EnhancedRequestService();